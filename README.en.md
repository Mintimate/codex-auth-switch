...
Optionally select comparison model B alongside reference model A. Both use the same account scope, token total, input share, and cache ratio. Each 7 / 30-day period shows no-cache and assumed-cache costs, plus B’s dollar and percentage difference relative to A. Swap A / B or expand the component costs and rates. Missing cached rates leave the affected scenario unavailable; missing usage is never treated as zero, and a zero baseline has no percentage difference. This compares rates at equal usage, not the actual tokens or quality each model would achieve on the same task.
