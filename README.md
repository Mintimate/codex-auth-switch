...
- 在订阅额度页按账号用量模拟近 7 / 30 天美元价值，自选参考模型与输入占比，对比零缓存和 90% 输入缓存
...
在账号库中选择“切换到此账号”。应用会校验目标账号，并原子替换 Codex 当前使用的 `auth.json`；请重启 Codex 后使切换生效。
...
订阅额度页的“订阅用量价值模拟”直接使用账号每日 Token 总数，按含今天的近 7 / 30 个 UTC 自然日汇总，可选择全部账号或单个账号。同一订阅的多个本地档案只统计一次，缺失用量不会当成零，刷新失败会保留并标注上次成功的数据。此功能不读取或归类本地会话。

当前接入的账号每日用量没有模型、输入/输出与缓存拆分，因此需选择参考模型、调整假设输入占比（初始为 80%），金额是情景模拟。零缓存将假设输入量全部按普通输入单价计价；90% 缓存将其中 90% 按缓存读取价计费，其余按普通输入价计费，输出量保持不变。无缓存单价时不展示该场景。当前协议中的单任务明细不能替代账号近 7 / 30 天的拆分。

单价来自 [OpenAI 公开价格](https://developers.openai.com/api/docs/pricing)。默认使用内置或本机缓存价格，点击“拉取最新价格”才下载公开文档，不携带认证凭据或用量；失败保留旧价格和日期。采用当前 Standard 短上下文文本标价，不是实际订阅账单，未计长上下文加价、Fast、缓存写入、工具调用、税费和折扣。价格文档解析与认证、额度兼容层独立；文档结构变化时不会猜测价格。
