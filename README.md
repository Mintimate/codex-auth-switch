...
- 通过独立的“订阅价值模拟”入口，按账号用量模拟近 7 / 30 天美元价值，提供任务场景预设、自选模型与比例、费用拆分，并可设为默认启动页
- 以紧凑账号列表展示订阅额度，支持搜索、状态筛选和按额度、恢复时间、用量、重置次数排序
- 展示额度窗口、恢复时间轴、近 7 天和近 30 天 Token，以及每日活跃热力图
- 进入用量、额度或价值模拟页时按需刷新对应数据；可关闭自动刷新，改为手动读取
...
“订阅价值模拟”是侧栏中的独立页面，进入后直接展示估算控件，也可在设置中将其设为默认启动页。页面复用订阅额度的账号用量数据源，可独立刷新账号用量，无需先打开订阅额度页。模拟直接使用账号每日 Token 总数，按含今天的近 7 / 30 个 UTC 自然日汇总，可选择全部账号或单个账号。同一订阅的多个本地档案只统计一次，缺失用量不会当成零，刷新失败会保留并标注上次成功的数据。此功能不读取或归类本地会话。

当前接入的账号每日用量没有模型、输入/输出与缓存拆分，因此需选择参考模型与试算比例。默认“编程多轮”为输入 99%、输入缓存 90%；“通用多轮”为输入 90%、输入缓存 50%；“新任务／少复用”为输入 80%、输入缓存 0%。这些预设只是试算起点，不是任务平均值或账号实测结果。输入占比与输入缓存比例均可通过滑块和数字输入调整，支持 0.1% 的步长。当前协议中的单任务明细不能替代账号近 7 / 30 天的拆分。

输入缓存比例指假设输入量中按缓存读取价计费的占比，输出不享受缓存折扣。页面对比零缓存与当前缓存假设，并拆分普通输入、缓存输入和输出的 Token 数与费用。只有假设缓存输入量大于零时，缺少缓存单价才会使当前缓存场景无法计算；普通输入和纯输出仍可按对应单价试算。

连续复用较长的历史上下文有利于缓存命中；新任务、上下文压缩、切换模型或较长间隔可能降低命中率。[OpenAI 缓存文档中的多轮 Agent](https://developers.openai.com/api/docs/guides/prompt-caching#multi-turn-agent) 给出过超过 90% 的命中率，但这是特定部署示例，不是编程或通用任务的平均值。请按实际工作方式调整假设。
...
“Token 用量”汇总本机会话元数据；“订阅额度”查询账号的在线额度；“订阅价值模拟”使用同一在线数据源中的账号用量进行试算。三个页面均可按需加载和刷新，打开账号页不会自动查询订阅额度。在“设置 → 用量与额度”中可关闭“进入页面时自动刷新”，之后通过各页面按钮手动读取或查询。
