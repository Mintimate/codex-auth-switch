...
遇到登录或额度刷新失败时，可在“实验室 → 诊断日志”启用收集，重新执行失败的操作，再点击“刷新列表”。支持仅看异常、展开脱敏响应和导出 JSON；同一轮额度查询或本地登录以查询组号关联，便于对照 App Server、兼容查询、重试和凭据刷新过程。

开关默认关闭且仅对本次运行生效，重启后自动关闭。记录请求方法和固定服务地址／RPC 方法、耗时、代理模式、状态码与白名单响应字段；不记录请求正文、认证头、Cookie、令牌、验证码、账号 ID、邮箱、授权链接或原始错误文本。服务端消息只可生成预设的粗略分类，不能单凭该分类确定根因。未知字段和任意字符串丢弃；额度百分比、用量计数等排查数据可能保留。HTML 响应只记录类型，登录成功响应中的认证凭据不进入日志。

日志仅保存到应用数据目录的 `diagnostic-logs.v1.json`，最多保留最近 7 天、500 条、1 MiB，在启动、读取及记录时淘汰旧数据。关闭后停止收集并保留历史；清空后，已在途请求不会重新写回。文件损坏时停止收集，需清空后重新启用；写入失败会提示，登录和额度查询继续正常工作。

“导出日志”在本机生成 `diagnostics/diagnostic-log-export.json` 并尝试打开所在目录，每次导出更新同一文件。导出包含全部保留记录及应用版本、系统类型，不受“仅看异常”筛选影响。清空会一并删除该导出文件，你另存或分享的副本由你管理。功能不上传日志；公开在线演示不能启用收集或导出。
